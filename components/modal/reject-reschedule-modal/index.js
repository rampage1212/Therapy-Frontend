...
import { getHoursDiff, isBefore24Hours, isRTLLayout } from "@/utils/helpers"
import { cancelPatientAppointment, patientRejectReshReq } from "@/utils/api"
import Spinner from "../spinner"
import warningIcon from "@/images/icons/red-warning-icon.svg"

const RejectRescheduleModal = ({
  isOpen = false,
  user,
  reqId,
  onClose,
  ...
  const handleCancelAppointment = async () => {
    ...
    try {
      const response = await patientRejectReshReq({
        user,
        reqId: reqId,
      })
      setIsSuccess(true)
    ...
    <>
          ...
          <div className={"cancel-appointment-modal"}>
            ...
            {isSuccess ? (
              <div className="success-wrapper">
                ...
                <h3>{t("confirmed")}</h3>
                <p>{t("appointment_has_canceled")}</p>
                <button
                  className="ok-btn"
                  onClick={() => {
                    handleClose()
            ...
            {!isSuccess && !isFaild ? (
              <div className="cancel-wrapper">
                ...
                <h3>{t("sure_reject_appointment")}</h3>
                {/* <p>
                  {t("sure_cancel_appointment_des")}
                </p> */}
                <div className="note">
                  <Image
                    src={warningIcon}
      ...
      <style jsx>{`
        ...
        .cancel-wrapper {
          @apply text-center;
          & h3 {
            @apply font-avenirBold text-black-0 text-2xl mb-3;
          }
          & p {
