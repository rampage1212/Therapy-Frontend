...
import { isAftereNow, isRTLLayout } from "@/utils/helpers"
import moment from "moment"
import { useEffect } from "react"
import {
  getAvailableAppointmentInDay,
  getAvailableDaysForDoctor,
  patientAcceptReshReq,
} from "@/utils/api"
...
const AcceptRecheduleModal = ({
  ...
  const handleBooking = async (data) => {
    ...
    try {
      const response = await patientAcceptReshReq({
        user: user,
        reqId: reqId,
  ...
  const onClick = () => {
    handleBooking({
      ...
      newApp: selectedAppointment || timeRef.current.value,
    })
  }
...
export default AcceptRecheduleModal
