...
import AdultsImage from "@/images/services/adults.png"
...
const AdultsTherapyContainer = ({ categoriesList, deviceType }) => {
  const factoredCategories = extractArrayData(categoriesList)
  const router = useRouter()
  const carouselRef = useRef(null)
  const individual = factoredCategories.find((cat) => {
    if (isRTLLayout(router)) {
      return cat?.id == "11"
    }
    return cat?.id == "9"
  })
  const cats = extractArrayData(individual?.categories?.data)
  const { t } = useTranslation()
  const [selectedCat, setSelectedCat] = useState("")
  const [doctorsList, setDoctorsList] = useState([])
  const contentData = [
    t("individual_therapy_desc_first"),
    t("individual_therapy_desc_second"),
  ]
  ...
  return (
    <div className="couple-therapy-container">
      <MainServiceCard
        cardImage={AdultsImage}
        title={t("adult_therapy_title")}
      ...
      <style jsx>{`
        ...
        .information {
          @apply flex justify-between items-start flex-col lg:flex-row lg:items-start mb-14;
        }
        .l-inf {
...
export default AdultsTherapyContainer
