...
const PsychiatryTherapyContainer = ({ categoriesList, deviceType }) => {
  const factoredCategories = extractArrayData(categoriesList)
  const router = useRouter()
  const carouselRef = useRef(null)
  const psychiatry = factoredCategories.find((cat) => {
    return cat?.id == "7"
  })
  const cats = extractArrayData(psychiatry?.categories?.data)
  const { t } = useTranslation()
  const [selectedCat, setSelectedCat] = useState("")
  const [doctorsList, setDoctorsList] = useState([])
  const contentData = [t("psychiatry_desc_first"), t("psychiatry_desc_second")]
  ...
  return (
    <div className="couple-therapy-container">
            ...
            >
              {deviceType !== "mobile"
                ...
                : doctorsList.map((doctor) => {
                    return <DoctorBookingCard key={doctor.id} doctor={doctor} />
                  })}
              {/* <CheckAllDoctors /> */}
              {/* <Image
      ...
      <style jsx>{`
        ...
        .information {
          @apply flex justify-between items-baseline flex-col lg:flex-row lg:items-start mb-14;
        }
        .l-inf {
          @apply text-[#2E3333] text-3xl font-medium max-w-xs mb-7 lg:mb-0;
        }
        .carousel-section {
          @apply flex justify-between items-center mt-10 lg:mt-0;
        }
        .carousel-controls {
          @apply flex gap-5;
          &.rtl {
        ...
        .cats-wrapper {
          @apply flex justify-center lg:justify-start gap-5 flex-wrap mb-7 lg:mb-10;
        }
      `}</style>
    </div>
  )
...
export default PsychiatryTherapyContainer
